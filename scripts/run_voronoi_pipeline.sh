...
if [ ! -f data/voronoi_states.npz ]; then
    $PYTHON -m diagnostics.voronoi_partition collect --n_episodes 200 2>&1 | tee outputs/csil_plus_plus/voronoi_collect_log.txt
else
...
$PYTHON -m diagnostics.voronoi_partition partition --k 4 2>&1 | tee outputs/csil_plus_plus/voronoi_partition_log.txt

# Step 3: Verify partition quality
echo ""
echo "=== Step 3: Verify partition quality ==="
$PYTHON -m diagnostics.voronoi_partition verify 2>&1 | tee outputs/csil_plus_plus/voronoi_verify_log.txt

# Step 4: Collect per-cell trajectory data (50 episodes per cell)
echo ""
echo "=== Step 4: Collect per-cell trajectory data (50 eps/cell) ==="
$PYTHON -m diagnostics.voronoi_partition collect-cell-data --n_episodes_per_cell 50 2>&1 | tee outputs/csil_plus_plus/voronoi_cell_collect_log.txt

# Step 5: Train K sub-policies
echo ""
echo "=== Step 5: Train K sub-policies (BC + CSIL++ PBRS) ==="
$PYTHON -m diagnostics.voronoi_partition train-sub-policies \
    --n_iterations 10 \
    --learning_rate 1e-7 \
    --max_kl 0.005 \
...
$PYTHON -m diagnostics.voronoi_partition route --n_episodes 50 2>&1 | tee outputs/csil_plus_plus/voronoi_route_eval_log.txt
