#!/usr/bin/env python3
"""SmolVLA Fine-tuning on Panda pick-and-place (local LeRobot dataset).

Adapted from smolvla_finetune.py to load a LOCAL LeRobot-format dataset at
/home/w/vla_workspace/data/panda_pickplace_lerobot/ instead of a remote Hub repo.

Training approach:
- Load pretrained SmolVLA base model (8D state/action, padded to 32D internally)
...
# Make the local LeRobot source importable before importing lerobot
...
from lerobot.datasets import LeRobotDataset, EpisodeAwareSampler, LeRobotDatasetMetadata
...
DATASET_REPO = "panda_pickplace"
DATASET_ROOT = "/home/w/vla_workspace/data/panda_pickplace_lerobot_v3"
OUTPUT_DIR = "/home/w/vla_workspace/outputs/smolvla_place_finetuned_v4"
...
def train(batch_size):
    """Run the full training loop. Returns the loss history list.

    Raises on error (caller may retry with a smaller batch size on OOM).
    """
    ...
    # ── Load LOCAL dataset metadata ───────────────────────────────────────────
    logger.info("Loading dataset metadata (local)...")
    ds_meta = LeRobotDatasetMetadata(DATASET_REPO, root=DATASET_ROOT)
    logger.info(
        f"Dataset metadata: {ds_meta.total_episodes} episodes, "
        f"{ds_meta.total_frames} frames, fps={ds_meta.fps}"
    )
    ...
    # ── Load LOCAL dataset with delta_timestamps for action chunking ──────────
    logger.info("Loading dataset with action chunking (local)...")
    ...
    # make_policy populates input/output features from ds_meta (8D state/action).
    # SmolVLA pads shorter vectors up to max_state_dim/max_action_dim (32), so
    # the 8D features are handled transparently and pretrained weights load fine.
    ...
    # Always use lerobot_collate_fn: the dataset items carry a `task` string
    # (added by the reader from tasks.parquet) that the tokenizer processor needs.
...
def main():
    """Entry point: try BATCH_SIZE=4, fall back to 2 on CUDA OOM."""
    try:
        train(BATCH_SIZE)
