#!/usr/bin/env python3
"""V65 pipeline — Distance penalty coefficient tuning (v14, k=1.0).

Root cause recap (V59-V64):
  V64 (v13: /max_episode_steps normalization) CRASHED to 5% at step 3000 —
  the WORST of all versions. The normalization made the distance penalty
  negligible (-0.003/step at 15cm), removing the positional gradient signal
  entirely. Without directional pressure, PPO optimized for easy proximity
  bonuses instead of reaching the target.

  The distance penalty serves DUAL roles:
    1. Hackable accumulation (bad): -750 total at 15cm/500 steps
    2. Directional gradient (good): per-step signal toward target

  V63 (k=10): too strong — accumulation crushes +200 success (gradient OK)
  V64 (k=0.02): too weak — no gradient, policy crashes to 5%
  V65 (k=1.0): sweet spot — gradient competitive, magnitude bounded

V65 core change (v14, in gym_env/panda_vla_env.py):
  reward -= 1.0 * block_target_dist  (was 10.0 in V63, 10.0/500 in V64)

  At 15cm: -0.15/step (comparable to +0.2 progress reward → gradient is
  competitive). Total over 500 steps: -75 (37.5% of +200 success, in the
  optimal 30-50% shaping range per reward design literature).
...
Single-variable isolation:
  V65 = V64 config + v14 coefficient (ONLY the distance penalty coefficient
  changed from 10.0/500 to 1.0).
...
Critical validation: step 3000 eval
  ...
  V65 (OPR + v12, k=1.0):            target > 45% at step 3000

  Health checks: bc_loss stable (~0.002), approx_kl stable (<0.01),
  explained_variance > 0.9.

Pipeline:
  1. Confirm D_succ.npz (reuse V62/V63/V64 buffer)
  2. Train from V59 best_hier with v14 reward + OPR (lambda_si=0.1)
  3. 5000 steps, hier eval every 1000, early stop + decoupling detection
  4. Final 50-ep eval of best_hier

Usage:
    python run_v65_pipeline.py
"""
import os
...
SAVE_PATH = os.path.join(WORKSPACE, "outputs/place_policy_v65")
TRAIN_LOG = os.path.join(WORKSPACE, "outputs/v65_train.log")
PIPELINE_LOG = os.path.join(WORKSPACE, "outputs/v65_pipeline.log")
...
TRAIN_CMD = [
    ...
    # V65: same OPR self-imitation buffer (single-variable: only distance penalty coefficient changed)
...
def run_training():
    log("=" * 60)
    log("V65 启动 — v14 距离惩罚系数调优 (k=1.0, gradient competitive)")
    log("=" * 60)
    log("起点：V59 best_hier (56% at 2.5k)")
    log("核心改进 (v14 in panda_vla_env.py):")
    log("  距离惩罚系数: V63 k=10 (太强,-750) → V64 k=0.02 (太弱,5%) → V65 k=1.0")
    log("  15cm梯度: -0.15/step (vs 进度+0.2/step, 信号竞争性)")
    log("  15cm/500步总量: -75 (vs +200成功, 37.5%在30-50%最佳区间)")
    log("  保留: v12 gating + early release penalty + OPR + 全部超参")
    log("单一变量: V65 = V64 config + v14 k=1.0 (仅距离惩罚系数改变)")
    log("对比：")
    ...
    log("  V65: 自模仿  λ=0.1  + v12 k=1.0       → 目标 >45% at step3000")
            ...
            if evals:
                ...
                if latest['step'] >= 3000 and last_report_step < 3000:
                    if latest['place_rate'] >= 50:
                        log("*** 关键验证: step3000 place_rate=%d%% >= 50%% — v14 k=1.0 确认有效! ***" % latest['place_rate'])
                    elif latest['place_rate'] > 45:
                        log("*** 关键验证: step3000 place_rate=%d%% > 45%% — v14 改善 (vs V63=40%%, V64=5%%) ***" % latest['place_rate'])
                    else:
                        log("*** 关键验证: step3000 place_rate=%d%% <= 45%% — v14 k=1.0 未达预期 ***" % latest['place_rate'])
            else:
...
def main():
    ...
    log("=" * 60)
    log("Step 1: 确认 D_succ.npz (复用 V62/V63/V64 的自模仿缓冲)")
    log("=" * 60)
    ...
    log("")
    log("=" * 60)
    log("Step 2: V65 训练 (v14 k=1.0 + v12 gating + OPR)")
    log("=" * 60)
    ...
    if os.path.exists(best_hier_model) and os.path.exists(best_hier_vecnorm):
        ...
        if result:
            log("\n结论：")
            v65 = result['place_rate']
            step3000_rate = next((e['place_rate'] for e in evals if e['step'] >= 3000), 0)
            if step3000_rate > 45 and v65 >= 56:
                log("  V65 成功! step3000=%d%% (>45%%), best_hier=%d%%" % (step3000_rate, v65))
                log("  → v14 k=1.0 + v12 gating 消除了解耦根因")
                log("  → 策略稳定, 可继续训练或部署")
            elif step3000_rate > 45:
                log("  V65 step3000=%d%% (>45%%, vs V63=40%%, V64=5%%) 但 best_hier=%d%%" % (step3000_rate, v65))
                log("  → v14 改善了解耦, 但策略未提升")
                log("  → 考虑增加训练步数或调整 OPR 权重")
            else:
                log("  V65 step3000=%d%% (<=45%%), best_hier=%d%%" % (step3000_rate, v65))
                log("  → v14 k=1.0 未达预期")
                log("  → 考虑 k=2.0 (更强梯度) 或在线 D_succ 扩展")
    else:
    ...
    log("  V65 best_hier:  %s (自模仿 λ=0.1, v12 k=1.0)" % (
