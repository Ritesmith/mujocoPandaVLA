#!/usr/bin/env python3
"""V64 pipeline — Distance penalty normalization (v13).

Root cause recap (V59-V63):
  V63 (v12 reward fix: gating + early release penalty) improved step 3000
  from V62's 35% to 40% — partial success. The gating closed the "drop
  block to end episode" hack, but decoupling persists because the distance
  penalty MAGNITUDE is still too large.

  At 15cm held for 500 steps: -10*dist/step = -1.5/step, accumulating to
  -750 total. The progress reward is only +0.2/step at 1cm XY progress.
  The policy needs 7.5cm/step progress to break even — physically
  impossible. The distance penalty crushes the progress signal, so PPO
  still finds it easier to reduce episode length than to reach the target.

V64 core change (v13, in gym_env/panda_vla_env.py):
  Normalize the distance penalty by max_episode_steps (500):
    reward -= 10.0 * block_target_dist / self.max_episode_steps
  Per-step: -0.02*dist (= -0.003 at 15cm). Total over 500 steps at 15cm:
  -1.5 (vs old -750). The +200 success reward now dominates the gradient,
  restoring the intended reward structure from the original pseudocode
  (-10*dist/max_steps) that was omitted in V63 for single-variable isolation.

  All other components unchanged: v12 gating, early release penalty, OPR
  self-imitation (lambda_si=0.1), PBRS, hyperparameters.

Single-variable isolation:
  V64 = V63 config + v13 distance normalization. The ONLY difference from
  V63 is the /max_episode_steps divisor on the distance penalty.
...
Critical validation: step 3000 eval
  ...
  V62 (OPR λ=0.1):                   35% at step 3000 (degraded)
  V63 (OPR + v12 gating):            40% at step 3000 (degraded, +5%)
  V64 (OPR + v12 + v13 normalize):   target > 45% at step 3000

  Health checks: bc_loss stable (~0.002), approx_kl stable (<0.01),
  explained_variance > 0.9.

Pipeline:
  1. Confirm D_succ.npz (reuse V62/V63 buffer)
  2. Train from V59 best_hier with v13 reward + OPR (lambda_si=0.1)
  3. 5000 steps, hier eval every 1000, early stop + decoupling detection
  4. Final 50-ep eval of best_hier

Usage:
    python run_v64_pipeline.py
"""
import os
...
SAVE_PATH = os.path.join(WORKSPACE, "outputs/place_policy_v64")
TRAIN_LOG = os.path.join(WORKSPACE, "outputs/v64_train.log")
PIPELINE_LOG = os.path.join(WORKSPACE, "outputs/v64_pipeline.log")
...
TRAIN_CMD = [
    ...
    # V64: same OPR self-imitation buffer as V62/V63 (single-variable: only distance penalty changed)
...
def run_training():
    log("=" * 60)
    log("V64 启动 — v13 距离惩罚归一化 (distance penalty normalization)")
    log("=" * 60)
    log("起点：V59 best_hier (56% at 2.5k)")
    log("核心改进 (v13 in panda_vla_env.py):")
    log("  距离惩罚: -10*dist → -10*dist/max_episode_steps (= -0.02*dist/step)")
    log("  量级修复: 15cm持物500步 old=-750 (压垮+200成功), new=-1.5 (成功主导)")
    log("  保留: v12 gating + early release penalty + OPR + 全部V63超参")
    log("单一变量: V64 = V63 config + v13 normalize (仅距离惩罚系数改变)")
    log("对比：")
    ...
    log("  V63: 自模仿  λ=0.1  + v12 gating         → 40% at step3000 (退化+5%)")
    log("  V64: 自模仿  λ=0.1  + v12 + v13 normalize→ 目标 >45% at step3000")
            ...
            if evals:
                ...
                if latest['step'] >= 3000 and last_report_step < 3000:
                    if latest['place_rate'] >= 50:
                        log("*** 关键验证: step3000 place_rate=%d%% >= 50%% — v13 归一化确认有效! ***" % latest['place_rate'])
                    elif latest['place_rate'] > 45:
                        log("*** 关键验证: step3000 place_rate=%d%% > 45%% — v13 改善 (vs V63=40%%) ***" % latest['place_rate'])
                    else:
                        log("*** 关键验证: step3000 place_rate=%d%% <= 45%% — v13 归一化未达预期 ***" % latest['place_rate'])
            else:
...
def main():
    ...
    log("=" * 60)
    log("Step 1: 确认 D_succ.npz (复用 V62/V63 的自模仿缓冲)")
    log("=" * 60)
    ...
    log("")
    log("=" * 60)
    log("Step 2: V64 训练 (v13 distance normalization + v12 gating + OPR)")
    log("=" * 60)
    ...
    if os.path.exists(best_hier_model) and os.path.exists(best_hier_vecnorm):
        ...
        if result:
            log("\n结论：")
            v64 = result['place_rate']
            step3000_rate = next((e['place_rate'] for e in evals if e['step'] >= 3000), 0)
            if step3000_rate > 45 and v64 >= 56:
                log("  V64 成功! step3000=%d%% (>45%%), best_hier=%d%%" % (step3000_rate, v64))
                log("  → v13 距离归一化 + v12 gating 消除了解耦根因")
                log("  → 策略稳定, 可继续训练或部署")
            elif step3000_rate > 45:
                log("  V64 step3000=%d%% (>45%%, vs V63=40%%) 但 best_hier=%d%%" % (step3000_rate, v64))
                log("  → v13 改善了解耦, 但策略未提升")
                log("  → 考虑增加训练步数或调整 OPR 权重")
            else:
                log("  V64 step3000=%d%% (<=45%%), best_hier=%d%%" % (step3000_rate, v64))
                log("  → v13 归一化未达预期")
                log("  → 考虑 V65: approach bonus 门控 (Option 2)")
    else:
        log("ERROR: best_hier/ model not found")

    log("\n基线对比：")
    log("  V59 best_hier:  56% (无BC, 2500步解耦, old reward)")
    log("  V60 best_hier:  50% (外部BC λ=0.3, step3000=15%%, old reward)")
    log("  V61 best_hier:  56% (外部BC λ=0.01, step3000=25%%, old reward)")
    log("  V62 best_hier:  56% (自模仿 λ=0.1, step3000=35%%, old reward)")
    log("  V63 best_hier:  50% (自模仿 λ=0.1, step3000=40%%, v12 gating)")
    log("  V64 best_hier:  %s (自模仿 λ=0.1, v12 + v13 normalize)" % (
