#!/usr/bin/env python3
"""V68 pipeline — Hover penalty (v17): first truly hack-free reward.

Root cause recap (V59-V67):
  V67 (v16: remove discrete bonuses + progressive reward 0.1/(1+dist)) CRASHED
  to 0% at step 3000 — WORST of all versions. DECOUPLING DETECTED: reward
  -82.6 → +3.04 (improving) while place_rate 50% → 0% (declining).

  V67 FAILURE ROOT CAUSE — the hover hack at dist≈0:
    v16's progressive reward (0.1/(1+dist)) was +0.1/step at dist=0.
    The k=5.0 distance penalty vanishes at dist=0 (-5*0=0), so hovering
    at the target was net POSITIVE (+0.1/step, ~+48/episode).

    Per-step net reward by distance (V67 v16):
      dist=0.15: -0.75 + 0.087 = -0.66 (navigate)     ✓
      dist=0.05: -0.25 + 0.095 = -0.16 (approach)     ✓
      dist=0.02: -0.10 + 0.098 = -0.00 (break-even)   ⚠
      dist=0.00:  0.00 + 0.100 = +0.10 (HOVER HACK!)  ✗

    Policy learned: approach target → hover at dist≈0 without releasing →
    farm progressive reward. Same structural flaw as discrete bonuses, just
    shifted from threshold-based to position-based.

  KEY LESSON: removing hackable discrete bonuses ≠ eliminating exploit space.
    Any position-based positive reward at dist=0 is hackable because the
    distance penalty is always 0 there. The fix must ensure net per-step
    reward is NEGATIVE at ALL distances, including dist=0.

V68 core changes (v17, in gym_env/panda_vla_env.py):
  1. REMOVE progressive reward entirely (was 0.1/(1.0+block_target_dist)).
     It was the source of the hover hack — +0.1/step at dist=0.
  2. ADD linear hover penalty: when is_holding and dist < 0.05,
     reward -= 0.5 * (0.05 - dist) / 0.05  (linearly 0→-0.5 as dist 0.05→0)

  Per-step net reward by distance (V68 v17):
    dist=0.15: -0.75 + 0    = -0.75 (navigate)         ✓
    dist=0.05: -0.25 + 0    = -0.25 (approach)         ✓
    dist=0.03: -0.15 - 0.20 = -0.35 (release pressure) ✓
    dist=0.01: -0.05 - 0.40 = -0.45 (strong pressure)  ✓
    dist=0.00:  0.00 - 0.50 = -0.50 (must release!)    ✓

  ALL distances have net-negative per-step reward. The policy is always
  penalized for holding, with penalty increasing as it approaches the
  target. The only escape from per-step penalty is to RELEASE — earning
  +200 success if at target, or -5 early release penalty if not.

  Boundary detail: dist=0.05 is a local minimum (derivative discontinuity:
  -5.0 outside vs +5.0 inside). Boundary hover (-0.25/step) is still
  net-negative. Releasing (+200) dominates when success_rate > 35%.
  V59 baseline success_rate=56% >> 35% threshold. Acceptable.

  Unchanged from V67:
    - k=5.0 distance penalty
    - v15 lowering bonus gate (xy_dist<0.10)
    - v12 gating (all continuous rewards on is_holding)
    - Early release penalty (-5)
    - Release bonus (+50, requires dist<0.05 AND on_table AND gripper_open)
    - Success bonus (+200, same conditions)
    - OPR self-imitation (lambda_si=0.1, D_succ.npz)
    - DECOUPLING DETECTION early stop
    - All hyperparameters

Single-variable isolation:
  V68 = V67 config + v17 (remove progressive reward, add hover penalty).
  ONLY the reward function changed.

  V68 is the FIRST version with NO hackable reward components:
    - V63-V66: discrete bonuses (+20/+50/+100) collectible without placement
    - V67: progressive reward (+0.1/step) collectible by hovering at dist=0
    - V68: ALL per-step rewards are negative → only +200/+50 terminal rewards
           are positive, and they require actual task completion

Critical validation: step 3000 eval — DECISIVE experiment
  V63 (k=10, +100 approach bonus):     40% at step 3000 (degraded)
  V64 (k=0.02, no gradient):            5% at step 3000 (crashed)
  V65 (k=1.0, Z-bias):                  5% at step 3000 (crashed)
  V66 (k=1.0, v15 gate):                5% at step 3000 (crashed)
  V67 (k=5.0, v16 progressive):         0% at step 3000 (crashed, hover hack)
  V68 (k=5.0, v17 hover penalty):  target > 45% at step 3000

  If V68 succeeds → reward structure was the root cause all along.
  If V68 fails   → PPO fine-tuning itself is destructive (not just reward).
                   Consider: freeze backbone, higher λ_bc, or abandon FT.

  Health checks: bc_loss stable (~0.002), approx_kl stable (<0.01),
  explained_variance > 0.9, step 1k place_rate ≥ 50% (V59 init preserved).

Pipeline:
  1. Confirm D_succ.npz (reuse V62-V67 buffer)
  2. Train from V59 best_hier with v17 reward + OPR (lambda_si=0.1)
  3. 5000 steps, hier eval every 1000, early stop + decoupling detection
  4. Final 50-ep eval of best_hier

Usage:
    python run_v68_pipeline.py
"""
import os
...
SAVE_PATH = os.path.join(WORKSPACE, "outputs/place_policy_v68")
TRAIN_LOG = os.path.join(WORKSPACE, "outputs/v68_train.log")
PIPELINE_LOG = os.path.join(WORKSPACE, "outputs/v68_pipeline.log")
...
TRAIN_CMD = [
    ...
    # V68: OPR self-imitation buffer (single-variable: only reward function changed)
...
def run_training():
    log("=" * 60)
    log("V68 启动 — v17 悬停惩罚 (首个无hackable组件的奖励结构)")
    log("=" * 60)
    log("起点：V59 best_hier (56% at 2.5k)")
    log("V67 失败根因 (hover hack at dist=0):")
    log("  v16 渐进奖励 0.1/(1+dist) 在 dist=0 时 = +0.1/step")
    log("  k=5.0 惩罚在 dist=0 时 = 0 (5*0=0)")
    log("  净 = +0.1/step → 悬停 hack (策略到达目标后不释放)")
    log("  V67 step3k: reward -82.6→+3.04 (优化) 但 place_rate 50%→0% (崩溃)")
    log("核心改进 (v17 in panda_vla_env.py):")
    log("  1. 移除渐进奖励 (0.1/(1+dist)) — 悬停 hack 的根源")
    log("  2. 添加线性悬停惩罚: dist<0.05 时 -0.5*(0.05-dist)/0.05 per step")
    log("  净 per-step 全距离为负:")
    log("    15cm=-0.75, 5cm=-0.25, 3cm=-0.35, 1cm=-0.45, 0cm=-0.50")
    log("  策略被持续推向释放 (唯一正收益是 +200 成功奖励)")
    log("  保留: k=5.0 + v15门控 + v12 gating + early release + OPR + 全部超参")
    log("  保留: DECOUPLING DETECTION 早停")
    log("单一变量: V68 = V67 config + v17 (仅奖励函数改变)")
    log("判决性实验: V68 是首个无hackable组件的版本")
    log("  成功 → 奖励结构是根因")
    log("  失败 → PPO微调本身有破坏性, 需转向优化器层面诊断")
    log("对比：")
    log("  V63: 自模仿 λ=0.1 + v12 k=10 + approach bonus  → 40% at step3000")
    log("  V64: 自模仿 λ=0.1 + v12 k=0.02                →  5% at step3000")
    log("  V65: 自模仿 λ=0.1 + v12 k=1.0                 →  5% at step3000")
    log("  V66: 自模仿 λ=0.1 + v12 k=1.0 + v15门控       →  5% at step3000")
    log("  V67: 自模仿 λ=0.1 + v12 k=5.0 + v16渐进奖励   →  0% at step3000 (hover hack)")
    log("  V68: 自模仿 λ=0.1 + v12 k=5.0 + v17悬停惩罚   → 目标 >45% at step3000")
            ...
            if evals:
                ...
                if latest['step'] >= 3000 and last_report_step < 3000:
                    if latest['place_rate'] >= 50:
                        log("*** 关键验证: step3000 place_rate=%d%% >= 50%% — v17 悬停惩罚确认有效! ***" % latest['place_rate'])
                    elif latest['place_rate'] > 45:
                        log("*** 关键验证: step3000 place_rate=%d%% > 45%% — v17 改善 (vs V64-V67≤5%%) ***" % latest['place_rate'])
                    else:
                        log("*** 关键验证: step3000 place_rate=%d%% <= 45%% — v17 未达预期, PPO微调本身可能有破坏性 ***" % latest['place_rate'])
            else:
...
def main():
    ...
    log("=" * 60)
    log("Step 1: 确认 D_succ.npz (复用 V62-V67 的自模仿缓冲)")
    log("=" * 60)
    ...
    log("")
    log("=" * 60)
    log("Step 2: V68 训练 (v17 悬停惩罚 + k=5.0 + v12 gating + OPR)")
    log("=" * 60)
    ...
    if os.path.exists(best_hier_model) and os.path.exists(best_hier_vecnorm):
        ...
        if result:
            log("\n结论：")
            v68 = result['place_rate']
            step3000_rate = next((e['place_rate'] for e in evals if e['step'] >= 3000), 0)
            if step3000_rate > 45 and v68 >= 56:
                log("  V68 成功! step3000=%d%% (>45%%), best_hier=%d%%" % (step3000_rate, v68))
                log("  → v17 悬停惩罚消除了最后一个 hackable 组件")
                log("  → 奖励结构是根因, 策略稳定, 可继续训练或部署")
            elif step3000_rate > 45:
                log("  V68 step3000=%d%% (>45%%, vs V64-V67≤5%%) 但 best_hier=%d%%" % (step3000_rate, v68))
                log("  → v17 改善了解耦, 但策略未提升")
                log("  → 考虑: 增加训练步数, 调整 OPR 权重, 或 500步评估频率")
            else:
                log("  V68 step3000=%d%% (<=45%%), best_hier=%d%%" % (step3000_rate, v68))
                log("  → v17 未达预期 — 首个无hackable奖励仍崩溃")
                log("  → 问题可能在 PPO 微调本身, 非奖励结构")
                log("  → 考虑: 冻结backbone, 提高λ_bc到0.3-0.5, 或放弃微调回V59部署")
    else:
    ...
    log("  V64 best_hier:  56% (自模仿 λ=0.1, step3000=5%%, v12 k=0.02)")
    log("  V65 best_hier:  56% (自模仿 λ=0.1, step3000=5%%, v12 k=1.0)")
    log("  V66 best_hier:  56% (自模仿 λ=0.1, step3000=5%%, v12 k=1.0+v15门控)")
    log("  V67 best_hier:  56% (自模仿 λ=0.1, step3000=0%%, v12 k=5.0+v16渐进奖励)")
    log("  V68 best_hier:  %s (自模仿 λ=0.1, v12 k=5.0+v17悬停惩罚 首个无hack)" % (
