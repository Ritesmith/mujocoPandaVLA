#!/usr/bin/env python3
"""V66 pipeline — Gate lowering bonus on XY proximity (v15).

Root cause recap (V59-V65):
  V65 (k=1.0) CRASHED to 5% at step 3000 — IDENTICAL to V64 (k=0.02).
  A 50x change in the distance penalty coefficient had NO effect on the
  failure mode. This proves the distance penalty coefficient is NOT the
  determining factor at low k.

  Diagnosis:
    1. block_target_dist IS 3D (np.linalg.norm(block_pos - target_pos)) —
       includes X, Y, Z. The "XY-only distance" hypothesis was DISPROVED.
    2. Height penalty (excess_height) is NOT active at 6-9cm lift — threshold
       is 10cm above table (0.32m). At V65's lift=6.1-9.1cm, excess_height=0.
       The height penalty did NOT cause the lift degradation.
    3. REAL ROOT CAUSE: the lowering bonus (+5*height_progress) fires
       UNCONDITIONALLY on Z descent — regardless of XY position. This creates
       a Z-descent bias: the policy earns +0.05/step by descending at the
       WRONG XY, then releases early (-5 one-time) instead of navigating to
       the target. At low k, this bias dominates the gradient.

  Per-step reward budget at k=1.0 (15cm from target):
    Distance penalty:  -0.15/step
    Block progress:    +0.20/step (XY-only, needs movement toward target)
    Lowering bonus:    +0.05/step (Z descent, UNCONDITIONAL)
    Net positive:      +0.10/step → policy rewarded for NOT reaching target

  At k=10 (V63, 40%): penalty -1.5/step dominates (+0.25 positive) → urgency
  At k=1.0 (V65, 5%):  penalty -0.15/step is dominated by +0.25 positive → no urgency

V66 core change (v15, in gym_env/panda_vla_env.py):
  Gate the lowering bonus on xy_dist < 0.10 (within 10cm of target XY).
  The policy must first navigate above the target, THEN descend. This
  removes the "descend anywhere" hack while preserving the legitimate
  Z-descent signal for final placement.

  Before (v14): reward += 5.0 * height_progress  (if descending)
  After  (v15): reward += 5.0 * height_progress  (if descending AND xy_dist<0.10)

  All other components unchanged: k=1.0 distance penalty, v12 gating,
  early release penalty, OPR self-imitation (lambda_si=0.1), PBRS, hyperparams.

Single-variable isolation:
  V66 = V65 config + v15 gating (ONLY the lowering bonus condition changed).
  If V66 survives step 3000 (where V64/V65 both crashed to 5%), the Z-descent
  bias is confirmed as the root cause.
...
Critical validation: step 3000 eval
  ...
  V66 (OPR + v12, k=1.0 + v15 gate):  target > 45% at step 3000

  Health checks: bc_loss stable (~0.002), approx_kl stable (<0.01),
  explained_variance > 0.9.

Pipeline:
  1. Confirm D_succ.npz (reuse V62-V65 buffer)
  2. Train from V59 best_hier with v15 reward + OPR (lambda_si=0.1)
  3. 5000 steps, hier eval every 1000, early stop + decoupling detection
  4. Final 50-ep eval of best_hier

Usage:
    python run_v66_pipeline.py
"""
import os
...
SAVE_PATH = os.path.join(WORKSPACE, "outputs/place_policy_v66")
TRAIN_LOG = os.path.join(WORKSPACE, "outputs/v66_train.log")
PIPELINE_LOG = os.path.join(WORKSPACE, "outputs/v66_pipeline.log")
...
TRAIN_CMD = [
    ...
    # V66: same OPR self-imitation buffer (single-variable: only lowering bonus gating changed)
...
def run_training():
    log("=" * 60)
    log("V66 启动 — v15 门控降低奖励 (gate on xy_dist<0.10)")
    log("=" * 60)
    log("起点：V59 best_hier (56% at 2.5k)")
    log("根因诊断 (V64/V65 失败分析):")
    log("  1. block_target_dist 是 3D (非XY-only) — 用户假设推翻")
    log("  2. 高度惩罚阈值 10cm, V65 lift=6-9cm 时未激活 — 非lift下降原因")
    log("  3. 真实根因: 降低奖励无条件触发 Z-下降偏置")
    log("     - 错误XY位置下降: +0.05/step (降低奖励) + 0 (XY进度)")
    log("     - k=1.0 距离惩罚 -0.15/step 太弱, 净 +0.10/step (正向!)")
    log("     - V64(k=0.02)与V65(k=1.0)崩溃模式相同 → k非决定因素")
    log("核心改进 (v15 in panda_vla_env.py):")
    log("  降低奖励门控: xy_dist<0.10 时才触发 (was: 无条件)")
    log("  策略必须先导航到目标上方, 再下降 — 消除'任意位置下降'hack")
    log("  保留: k=1.0 距离惩罚 + v12 gating + early release + OPR + 全部超参")
    log("单一变量: V66 = V65 config + v15 门控 (仅降低奖励条件改变)")
    log("对比：")
    ...
    log("  V66: 自模仿 λ=0.1 + v12 k=1.0 + v15门控 → 目标 >45% at step3000")
            ...
            if evals:
                ...
                if latest['step'] >= 3000 and last_report_step < 3000:
                    if latest['place_rate'] >= 50:
                        log("*** 关键验证: step3000 place_rate=%d%% >= 50%% — v15 门控确认有效! ***" % latest['place_rate'])
                    elif latest['place_rate'] > 45:
                        log("*** 关键验证: step3000 place_rate=%d%% > 45%% — v15 改善 (vs V64=5%%, V65=5%%) ***" % latest['place_rate'])
                    else:
                        log("*** 关键验证: step3000 place_rate=%d%% <= 45%% — v15 门控未达预期 ***" % latest['place_rate'])
            else:
...
def main():
    ...
    log("=" * 60)
    log("Step 1: 确认 D_succ.npz (复用 V62-V65 的自模仿缓冲)")
    log("=" * 60)
    ...
    log("")
    log("=" * 60)
    log("Step 2: V66 训练 (v15 门控降低奖励 + v12 gating + k=1.0 + OPR)")
    log("=" * 60)
    ...
    if os.path.exists(best_hier_model) and os.path.exists(best_hier_vecnorm):
        ...
        if result:
            log("\n结论：")
            v66 = result['place_rate']
            step3000_rate = next((e['place_rate'] for e in evals if e['step'] >= 3000), 0)
            if step3000_rate > 45 and v66 >= 56:
                log("  V66 成功! step3000=%d%% (>45%%), best_hier=%d%%" % (step3000_rate, v66))
                log("  → v15 门控降低奖励消除了解耦根因 (Z-下降偏置)")
                log("  → 策略稳定, 可继续训练或部署")
            elif step3000_rate > 45:
                log("  V66 step3000=%d%% (>45%%, vs V64=5%%, V65=5%%) 但 best_hier=%d%%" % (step3000_rate, v66))
                log("  → v15 门控改善了解耦, 但策略未提升")
                log("  → 考虑增加训练步数或调整 OPR 权重")
            else:
                log("  V66 step3000=%d%% (<=45%%), best_hier=%d%%" % (step3000_rate, v66))
                log("  → v15 门控未达预期, Z-下降偏置可能非唯一根因")
                log("  → 考虑: k=2.0 (更强梯度) 或 在线 D_succ 扩展 或 块进度也门控")
    else:
    ...
    log("  V66 best_hier:  %s (自模仿 λ=0.1, v12 k=1.0 + v15 门控降低奖励)" % (
